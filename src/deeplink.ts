...
export function getNativeDeepLink(eventoID?: string | number | null, rota?: string, extraParams?: string | URLSearchParams): string {
  let rotaFinal = rota || (eventoID ? 'Encontros_Corridas_Geral' : 'Pagina_inicio');
  if (rotaFinal.startsWith('/')) {
    rotaFinal = rotaFinal.substring(1);
  }
  let base = `vroomapp://vroomapp.pt/${rotaFinal}`;
  let queryParts: string[] = [];
  if (eventoID !== undefined && eventoID !== null && eventoID !== '') {
    queryParts.push(`eventoID=${eventoID}`);
  }
  if (extraParams) {
    const params = typeof extraParams === 'string' ? new URLSearchParams(extraParams.startsWith('?') ? extraParams : '?' + extraParams) : extraParams;
    for (const [key, value] of params.entries()) {
      if (key !== 'eventoID' && key !== 'id' && key !== 'rota') {
...
export function renderDeeplinkUI() {
  ...
  const rotaRecebida = urlParams.get('rota') || (eventoID ? 'Encontros_Corridas_Geral' : 'Pagina_inicio');

  const container = document.getElementById('deeplink-ui');
  if (!container) return;

  // 2. Construção do Custom URI Scheme Nativo
  const deepLink = getNativeDeepLink(eventoID, rotaRecebida, window.location.search);
  ...
  if (eventoID) {
    // Injeção do layout e botões de ação para Evento
    container.innerHTML = `
      <div class="max-w-lg w-full mx-auto space-y-3">
        ...
        <div class="deeplink-card bg-[#171A21] border border-[#262B37] rounded-3xl p-6 sm:p-8 text-center space-y-6 shadow-2xl relative overflow-hidden">
          ...
          <div class="actions flex flex-col sm:flex-row items-center justify-center gap-3 pt-3">
            <a id="deepLinkBtn" href="${deepLink}" class="btn btn-primary w-full sm:w-auto px-6 py-3.5 bg-brand-blue hover:bg-blue-600 active:scale-98 text-white font-bold text-sm sm:text-base rounded-xl transition-all duration-200 flex items-center justify-center gap-2.5 shadow-lg shadow-brand-blue/25 cursor-pointer">
    ...
    // Ecrã de fallback com mensagem de boas-vindas sem erro de ID não encontrado
    container.innerHTML = `
      <div class="max-w-lg w-full mx-auto space-y-3">
        <div class="flex items-center justify-start px-1">
          <a href="/" class="inline-flex items-center gap-2 px-3.5 py-1.5 bg-[#171A21] hover:bg-[#262B37] text-slate-300 hover:text-white rounded-xl border border-[#262B37] text-xs font-semibold transition-all shadow-md">
        ...
        <div class="deeplink-card bg-[#171A21] border border-[#262B37] rounded-3xl p-6 sm:p-8 text-center space-y-6 shadow-2xl relative overflow-hidden">
          ...
          <div class="relative z-10 space-y-4">
            <div class="w-16 h-16 bg-brand-blue/10 border border-brand-blue/30 rounded-2xl flex items-center justify-center mx-auto text-brand-blue">
            ...
            <div>
              <h2 class="text-xl sm:text-2xl font-bold text-white tracking-tight">Bem-vindo ao Vroom.pt</h2>
              <p class="text-xs sm:text-sm text-slate-300 mt-2 leading-relaxed">
            ...
            <div class="actions flex flex-col sm:flex-row items-center justify-center gap-3 pt-3">
              ...
              <a href="/" class="btn-secondary w-full sm:w-auto px-6 py-3.5 bg-[#12151D] hover:bg-[#1D212B] text-slate-200 border border-[#262B37] font-semibold text-sm rounded-xl transition-all duration-200 flex items-center justify-center gap-2 cursor-pointer">
                🌐 Continuar no Website
              </a>
            </div>
